...
app.use((req,res, next) => {
  ...
  if ("playlist" in req.session) {
    console.log("IN CONVERSION MIDDLEWARE");
    req.session.playlist.forEach(song => {
      newPlaylist.push(Song.makeSong(song));
    });
  }
  if ("prevSong" in req.session) {
    req.session.prevSong = Song.makeSong(req.session.prevSong);
  }
...
app.post('/videoFinished', (req, res) => {
  ...
  let song = Playlist.getSongByVideoId(req.session.playlist,videoId);
  song.toggleCurSong();
  let songRemoved = req.session.playlist.shift();
  req.session.playlist[0].toggleCurSong();
