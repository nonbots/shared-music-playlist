...
app.get("/playlist", (req, res) => {
   ...
   let videoIds = playlist.getVideoIds();
    res.render("playlist", {userLogin: req.session.user, playlist: playlist.playlist, videoId: req.session.videoId, videoIds: videoIds});
});
...
app.post("/playlist/edit-song/delete/:songId", (req,res) => {
    let songId = Number(req.params.songId);
   for (let i = 0; i < playlist.playlist.length; i += 1) {
       if( playlist.playlist[i].id === songId) {
           playlist.playlist.splice(i, 1);
           break;
        }
   }
    res.redirect("/playlist");
